...
## 2024-09-13 - [Fix openvino mock import in tests]
**Learning:** When writing tests that simulate missing dependencies by patching `sys.modules` to `None`, ensure the mocked dictionary key exactly matches the module being tested (e.g., `patch.dict('sys.modules', {'openvino': None})` for OpenVINO, rather than mistakenly duplicating keys like `'jax'`).
**Action:** Always verify the keys in `patch.dict("sys.modules", ...)` to ensure they align with the logic being tested.

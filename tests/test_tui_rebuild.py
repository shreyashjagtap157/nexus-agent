...
class TestTuiRebuildFeatures(unittest.TestCase):
    ...
    def test_welcome_dashboard_rendering_wide(self):
        ...
        with (
            ...
            # Ensure model name, CPU/GPU stats, RAM stats, token IO, context,
            # git diff and processes are present
    ...
    def test_workspace_session_auto_resume(self):
        """Verify session orchestrator resumes the last session unless new_session is True."""
    ...
    def test_custom_model_tuning_parameters(self):
        """Verify that model tuning parameters are validated and passed to LocalEngine."""
        from nexus_agent.llm.runtime_manager import RuntimeManager

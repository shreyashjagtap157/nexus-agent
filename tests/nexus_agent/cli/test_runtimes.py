...
class TestCheckTpu(unittest.TestCase):
    ...
    def test_jax_imported(self):
        with patch.dict(
            "sys.modules",
            {
                "jax": MagicMock(__file__="/path/jax/__init__.py"),
                "jaxlib": MagicMock(__file__="/path/jaxlib/__init__.py"),
            },
        ):
            runtimes = _check_tpu()
            self.assertEqual(len(runtimes), 1)
...
class TestScanRuntimes(unittest.TestCase):
    ...
    @patch(
        "nexus_agent.cli.runtimes._check_cpu",
        return_value=[
            RuntimeInfo(name="CPU default", provider="local", available=True, priority=10)
        ],
    )
    def test_scan_runtimes_smoke(
        self,
        mock_cpu,
        mock_cuda,
        mock_vulkan,

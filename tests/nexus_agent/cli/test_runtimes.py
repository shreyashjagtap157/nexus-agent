...
class TestCheckCuda(unittest.TestCase):
    ...
    @patch(
        ...
        side_effect=lambda x: "/usr/bin/llama-server"
        if x in ("llama-cli", "llama-server")
        else None,
    )
...
class TestCheckVulkan(unittest.TestCase):
    ...
    def test_no_vulkan(self, mock_which):
        with (
            ...
            patch.dict(
                "sys.modules",
                {"onnxruntime": MagicMock(__file__="/path/onnxruntime/__init__.py")},
            ),
            patch("os.name", "nt"),
        ):
            runtimes = _check_vulkan()
...
class TestCheckTpu(unittest.TestCase):
    ...
    def test_jax_imported(self):
        with patch.dict(
            "sys.modules", {"jax": MagicMock(__file__="/path/jax/__init__.py")}
        ):
            runtimes = _check_tpu()
            self.assertEqual(len(runtimes), 1)

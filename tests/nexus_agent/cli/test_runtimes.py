...
class TestCheckCpu(unittest.TestCase):
    ...
    def test_llama_cpp_python_imported(self, mock_which, mock_shutil):
        with patch.dict("sys.modules", {"llama_cpp": MagicMock(__file__="/path/llama_cpp/__init__.py")}):  # noqa: E501
            runtimes = _check_cpu()
            names = [r.name for r in runtimes]
    ...
    def test_transformers_imported(self, mock_which, mock_shutil):
        with patch.dict("sys.modules", {"transformers": MagicMock(__file__="/path/transformers/__init__.py")}):  # noqa: E501
            runtimes = _check_cpu()
            names = [r.name for r in runtimes]
...
class TestCheckCuda(unittest.TestCase):
    ...
    @patch("nexus_agent.cli.runtimes._which", side_effect=lambda x: "/usr/bin/llama-server" if x in ("llama-cli", "llama-server") else None)  # noqa: E501
...
class TestCheckVulkan(unittest.TestCase):
    ...
    @patch("nexus_agent.cli.runtimes._which", side_effect=lambda x: "/usr/bin/llama-vulkan" if x == "llama-vulkan" else None)  # noqa: E501
...
class TestCheckOpenvino(unittest.TestCase):
    ...
    def test_openvino_imported(self):
        with patch.dict("sys.modules", {"openvino": MagicMock(__file__="/path/openvino/__init__.py")}):  # noqa: E501
...
class TestScanRuntimes(unittest.TestCase):
    ...
    def test_all_checkers_called(self, mock_trt, mock_ext, mock_mlx, mock_sglang, mock_vllm,
                                 mock_tpu, mock_openvino, mock_rocm, mock_vulkan, mock_cuda, mock_cpu):  # noqa: E501
        mock_cpu.return_value = [RuntimeInfo(name="CPU", provider="local", available=True, priority=10)]  # noqa: E501
        mock_cuda.return_value = [RuntimeInfo(name="CUDA", provider="cuda", available=True, priority=90)]  # noqa: E501
        mock_vulkan.return_value = [RuntimeInfo(name="Vulkan", provider="vulkan", available=True, priority=70)]  # noqa: E501
        mock_rocm.return_value = [RuntimeInfo(name="ROCm", provider="rocm", available=True, priority=65)]  # noqa: E501
        mock_openvino.return_value = [RuntimeInfo(name="OpenVINO", provider="openvino", available=True, priority=45)]  # noqa: E501
        mock_tpu.return_value = [RuntimeInfo(name="TPU", provider="tpu", available=True, priority=35)]  # noqa: E501
...
class TestFormatRuntimeList(unittest.TestCase):
    ...
    def test_format_multiple(self):
        runtimes = [
            RuntimeInfo(name="CUDA", provider="cuda", available=True, path="/usr/bin/nvcc", version="12.1", description="NVIDIA CUDA", priority=90),  # noqa: E501
            RuntimeInfo(name="CPU", provider="local", available=True, description="Default CPU", priority=10),  # noqa: E501
        ]
    ...
    def test_format_single_builtin(self):
        runtimes = [
            RuntimeInfo(name="CPU (default)", provider="local", available=True, path="builtin", priority=10),  # noqa: E501
        ]
        result = format_runtime_list(runtimes)

    ...
    def execute(self, **kwargs: Any) -> str:
        ...
        try:
            ...
            if self.provider:
                ...
                system = (
                    "You are a git expert. Analyze the provided git diff and output a single, "
                    "perfect conventional commit message. Use conventional commits formatting "
                    "(e.g. feat: add task graph module, fix: resolve context timeout issue, "
                    "docs: update readme). Do NOT add extra explanations, "
...
class PRGeneratorTool(Tool):
    ...
    def execute(self, base_branch: str = "main", **kwargs: Any) -> str:
        try:
            ...
            if self.provider:
                system = (
                    "You are a principal engineer. Create a compelling, professional "
                    "Pull Request title and description "
                    "based on the provided commit messages and change stats. "
                    "Format the description in Markdown with:\n"
...
class CIAnalyzerTool(Tool):
    ...
    def execute(self, log_text: str, **kwargs: Any) -> str:
        ...
        if self.provider:
            system = (
                "You are an expert DevOps engineer and debugger. Read the provided "
                "CI log output block. "
                "Diagnose exactly why the CI failed, highlight the root error file/line/context, "
                "and provide the exact corrective actions to fix it."
            )
            res = self.provider.chat_completion(
                [
                    Message(role=Role.SYSTEM, content=system),

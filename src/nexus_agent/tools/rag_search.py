...
class RepositoryRAGTool(Tool):
    ...
    def _reindex_workspace(self) -> None:
        ...
        # ⚡ Bolt Optimization: Replace os.walk with iter_files (os.scandir)
        # Prevents loading massive directory trees into memory before indexing
        for file_path in iter_files(Path(self.workspace), exclude_dirs=exclude_dirs):

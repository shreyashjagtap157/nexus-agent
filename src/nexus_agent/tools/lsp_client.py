...
class LSPClientTool(Tool):
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        return (
            f"Action '{action}' is not supported by the local fallback (no LSP server installed)."
        )
    ...
    def _dispatch_lsp(
        ...
        if action == "format":
            ...
            return (
                "Formatting edits available from the server. Apply via edit_file "
                "with the returned WorkspaceEdit."
            )
    ...
    def _run_diagnostics(self, path: Path, content: str) -> str:
        ...
        if path.suffix.lower() == ".py":
            try:
                ...
                return (
                    f"✅ Diagnostics OK! No syntax compile errors found in Python file: {path.name}"
                )
            ...
            for idx, line in enumerate(content.splitlines()):
                ...
                for char_idx, char in enumerate(line):
                    ...
                    if char == "'":
                        ...
                        if not stack:
                            return (
                                f"❌ SYNTAX ERROR: Unexpected closing bracket '{char}' "
                                f"at line {idx + 1}, column {char_idx + 1}"
                            )
                        last_char, last_line, last_col = stack.pop()
                        if last_char != bracket_map[char]:
                            return (
                                f"❌ SYNTAX ERROR: Unmatched brackets: '{last_char}' "
                                f"opened at line {last_line} but closed with '{char}' "
                                f"at line {idx + 1}"
                            )
                    prev_char = char
            if stack:
                last_char, last_line, last_col = stack.pop()
                return (
                    f"❌ SYNTAX ERROR: Unclosed bracket '{last_char}' opened "
                    f"at line {last_line}, column {last_col + 1}"
                )

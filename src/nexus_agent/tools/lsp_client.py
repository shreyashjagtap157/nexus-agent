...
class LSPClientTool(Tool):
    ...
    def execute(self, action: str, file: str, line: int = 1,
        ...
        return f"Action '{action}' is not supported by local fallback."
    ...
    def _dispatch_lsp(
        ...
        if action == "references":
            result = client.request(
                ...
                {"textDocument": text_doc, "position": position, "context": {"includeDeclaration": 1}},
            )
        ...
        if action == "format":
            ...
            return "Formatting edits available. Apply via edit_file with the returned WorkspaceEdit."
    ...
    def _render_symbols(symbols: Any) -> str:
        ...
        def flatten(items: list[dict[str, Any]], depth: int = 0) -> list[str]:
            ...
            for item in items:
                ...
                r = item.get("location", {}).get("range", {}) or item.get("range", {})
        ...
        return "### Document symbols:\n" + "\n".join(body) if body else "No symbols reported."
    ...
    def _run_diagnostics(self, path: Path, content: str) -> str:
        ...
        if path.suffix.lower() == ".py":
            try:
                ...
                return f"✅ Diagnostics OK! No syntax errors found in file: {path.name}"
            ...
            for idx, line in enumerate(content.splitlines()):
                ...
                for char_idx, char in enumerate(line):
                    ...
                    if char == "'":
                        ...
                        if not stack:
                            return f"❌ SYNTAX ERROR: Unexpected closing bracket '{char}' at line {idx + 1}"
                        last_char, last_line, last_col = stack.pop()
                        if last_char != bracket_map[char]:
                            return f"❌ SYNTAX ERROR: Unmatched bracket '{last_char}' closed with '{char}' at line {idx + 1}"
                    prev_char = char
            if stack:
                last_char, last_line, last_col = stack.pop()
                return f"❌ SYNTAX ERROR: Unclosed bracket '{last_char}' opened at line {last_line}"

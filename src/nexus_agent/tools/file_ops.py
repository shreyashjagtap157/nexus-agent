    ...
    def execute(
        ...
        if start_line is not None or end_line is not None:
            ...
            if start < 0 or start >= total_lines:
                return f"Error: start_line {start_line} is out of range (file has "
                    f"{total_lines} lines)."
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        if any(bad in pattern for bad in ["*+", "++", "?+", "*?", "+?", "??", "**"]):
            return "Error: Dangerous regular expression pattern "
                "(nested/consecutive quantifiers detected)."
        ...
        if re.search(r"\[\^[^\]]*\][\*\+\?][\*\+\?]", pattern):
            return "Error: Dangerous regular expression pattern "
                "(consecutive quantifiers on character class)."

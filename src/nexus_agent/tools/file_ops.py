...
def scandir_rglob(path, pattern, limit=None, _matches=None):
    """
    ⚡ Bolt: Lazily evaluate recursive file matching using os.scandir instead of pathlib.rglob.
    Avoids intermediate Path object creation and significantly speeds up directory traversals.
    """
    import fnmatch
    import os
    from pathlib import Path
    ...
    try:
        with os.scandir(str(path)) as it:
            for entry in it:
                if limit is not None and len(_matches) >= limit:
                    break
                try:
                    if entry.is_dir(follow_symlinks=False):
                        ...
                        if entry.name in skip_dirs:
                            continue
                        scandir_rglob(Path(entry.path), pattern, limit, _matches)
                    elif entry.is_file():
                        if fnmatch.fnmatch(entry.name, pattern):
                            _matches.append(Path(entry.path))
                except OSError:
                    continue

...
class SearchFilesTool(Tool):
    ...
    def execute(self, pattern: str, path: str | None = None,
        ...
        if any(bad in pattern for bad in ["*+", "++", "?+", "*?", "+?", "??", "**"]):
            return "Error: Dangerous regular expression pattern (nested/consecutive quantifiers)."
        ...
        if re.search(r'\[\^[^\]]*\][\*\+\?][\*\+\?]', pattern):
            return "Error: Dangerous regular expression pattern (consec quantifiers on char class)."

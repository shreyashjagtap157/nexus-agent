    ...
    def execute(self, path: str, start_line: int | None = None,
        ...
        if start_line is not None or end_line is not None:
            ...
            if start < 0 or start >= total_lines:
                return f"Error: start_line {start_line} is out of range (file has {total_lines} lines)."  # noqa: E501  # noqa: E501
...
class SearchFilesTool(Tool):
    ...
    def execute(self, pattern: str, path: str | None = None,
        ...
        if any(bad in pattern for bad in ["*+", "++", "?+", "*?", "+?", "??", "**"]):
            return "Error: Dangerous regular expression pattern (nested/consecutive quantifiers detected)."  # noqa: E501  # noqa: E501
        ...
        if re.search(r'\[\^[^\]]*\][\*\+\?][\*\+\?]', pattern):
            return "Error: Dangerous regular expression pattern (consecutive quantifiers on character class)."  # noqa: E501  # noqa: E501
                ...
                for entry in it:
                    try:
                        if entry.is_dir(follow_symlinks=False):
                            ...
                            if entry.name.startswith(".") and entry.name not in {".env", ".gitignore"}:  # noqa: E501  # noqa: E501
                                continue
                            skip_dirs = {"node_modules", "__pycache__", ".git", "venv", ".venv", "dist", "build"}  # noqa: E501  # noqa: E501

...
class RenameTool(Tool):
    ...
    def description(self) -> str:
        return (
            "AST-based find-and-replace to safely rename symbols/variables "
            "across scope in a file."
        )
    ...
    def execute(self, file_path: str, old_symbol: str, new_symbol: str, **kwargs: Any) -> str:
        ...
        try:
            if target.stat().st_size > self._MAX_FILE_SIZE:
                return (
                    f"Error: File too large for rename "
                    f"({target.stat().st_size / 1024 / 1024:.1f}MB > 10MB)."
                )
        except OSError as e:

...
class ImportGraphTool(Tool):
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            "action": {
                "type": "string",
                "description": "The action to take: 'build' (generate full graph) or 'find_dependents' (find files depending on target)",  # noqa: E501  # noqa: E501
            },
            "target": {
                "type": "string",
                "description": "Module name or file path to check dependents of (required if action='find_dependents')",  # noqa: E501  # noqa: E501
                "required": False,
            }
        }
    ...
    def execute(self, action: str, target: str = "", **kwargs: Any) -> str:
        ...
        if action == "build":
            ...
            return f"### Modules importing '{target}':\n" + "\n".join(f"- `{d}`" for d in dependents)  # noqa: E501  # noqa: E501
...
class CallGraphTool(Tool):
    ...
    @property
    def description(self) -> str:
        return "Generates a call-graph for Python functions inside a file or traces where a function is called."  # noqa: E501  # noqa: E501
    ...
    def execute(self, file_path: str, trace_function: str = "", **kwargs: Any) -> str:
        ...
        if trace_function:
            ...
            if not callers:
                return f"No function calls targeting '{trace_function}' detected inside `{file_path}`."  # noqa: E501  # noqa: E501
            return f"### Function '{trace_function}' is called by:\n" + "\n".join(f"- `{c}`" for c in callers)  # noqa: E501  # noqa: E501
            ...
            for caller, callees in call_map.items():
                if callees:
                    lines.append(f"- `{caller}` calls: {', '.join(f'`{c}`' for c in sorted(callees))}")  # noqa: E501  # noqa: E501
            return "\n".join(lines)
...
class RenameTool(Tool):
    ...
    @property
    def description(self) -> str:
        return "AST-based find-and-replace to safely rename symbols/variables across scope in a file."  # noqa: E501  # noqa: E501
    ...
    def execute(self, file_path: str, old_symbol: str, new_symbol: str, **kwargs: Any) -> str:
        ...
        try:
            if target.stat().st_size > self._MAX_FILE_SIZE:
                return f"Error: File too large for rename ({target.stat().st_size / 1024 / 1024:.1f}MB > 10MB)."  # noqa: E501  # noqa: E501
        except OSError as e:
        ...
        try:
            ...
            return f"Successfully renamed '{old_symbol}' to '{new_symbol}' ({replacements} replacements) in `{file_path}`."  # noqa: E501  # noqa: E501
        except (SyntaxError, OSError, ValueError, UnicodeDecodeError) as e:
            ...
            try:
                ...
                os.replace(tmp_path, str(target))
                return f"Successfully updated symbol '{old_symbol}' to '{new_symbol}' ({count} regex replacements) in `{file_path}`."  # noqa: E501  # noqa: E501

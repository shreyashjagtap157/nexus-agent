...
class CallGraphTool(Tool):
    ...
    def execute(self, file_path: str, trace_function: str = "", **kwargs: Any) -> str:
        ...
        if trace_function:
            ...
            if not callers:
                return (
                    f"No function calls targeting '{trace_function}' detected inside `{file_path}`."  # noqa: E501
                )
...
class RenameTool(Tool):
    ...
    def description(self) -> str:
        return (
            "AST-based find-and-replace to safely rename symbols/variables across scope in a file."  # noqa: E501
        )

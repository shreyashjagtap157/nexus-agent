...
def session_checkpoint(description: str) -> None:
    ...
    # ⚡ Bolt: Replace slow Path.rglob with fast os.scandir via iter_files  # noqa: E501
    files_iter = iter_files(Path.cwd())
    files = list(itertools.islice((str(f) for f in files_iter if f.name.endswith(".py")), 20))

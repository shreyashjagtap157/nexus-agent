...
@click.option("--verbose", is_flag=True, default=False, help="Show verbose debug output")  # noqa: E501
        ...
        while stack:
            ...
            try:
                for entry in os.scandir(cur):
                    ...
                    elif entry.is_file(follow_symlinks=False) and entry.name.endswith('.py'):  # noqa: E501
                        yield entry.path
            except OSError:
                pass

    files = list(islice(_fast_rglob_py(Path.cwd()), 20))

...
def session_checkpoint(description: str) -> None:
    ...
    # ⚡ Bolt: Use os.scandir instead of pathlib.rglob for faster file traversal without intermediate Path objects  # noqa: E501
    def _fast_scan():
        stack = [os.getcwd()]
        while stack:
            try:
    ...
    files = list(itertools.islice(_fast_scan(), 20))
...
def hardware() -> None:
    ...
    lines = [
        ...
        "",
        f"[bold green]Recommended max model size:[/bold green] {hw.get('recommended_model_size', 'unknown')}",  # noqa: E501  # noqa: E501
    ]

...
class SecretScanner:
    ...
    def scan(self) -> list[SecretMatch]:
        ...
        try:
            # ⚡ Bolt Optimization: Replace os.walk with iter_files (os.scandir) to drastically
            # reduce memory overhead and path-join operations when scanning large repositories.
            for file_path in iter_files(
                self.workspace, exclude_dirs=self.EXCLUDE_DIRS, include_hidden=True
            ):

            ...
            for f in files:
                if (self.workspace / f).exists():
                    if fw == "pytest-fallback":
                        # PERFORMANCE OPTIMIZATION: Avoid materializing a full list of all test files
                        # just to check if the directory contains any tests. Use next() for lazy evaluation.
                        has_tests = next((self.workspace / "tests").glob("**/*.py"), None) is not None
                        if (self.workspace / "pyproject.toml").exists() or has_tests:
                            return "pytest"
                    else:
                        return fw.replace("-fallback", "")

        # PERFORMANCE OPTIMIZATION: Avoid materializing a full list of all python files
        has_src = next(self.workspace.glob("*.py"), None) is not None or (
            next((self.workspace / "src").glob("**/*.py"), None) is not None if (self.workspace / "src").exists() else False
        )
        if has_src:
            return "unittest"
...
class SecretScanner:
    ...
    def scan(self) -> list[SecretMatch]:
        ...
        try:
            # PERFORMANCE OPTIMIZATION: Replacing slow pathlib.rglob or os.walk traversals with
            # iter_files() which uses a custom recursive os.scandir implementation under the hood,
            # yielding significant speedups for repeated traversals on large codebases.

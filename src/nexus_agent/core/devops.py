...
class TestRunner:
    ...
    def detect_framework(self) -> str | None:
            ...
            for f in files:
                if (self.workspace / f).exists():
                    if fw == "pytest-fallback":
                        if (self.workspace / "pyproject.toml").exists() or list(
        ...
        if list(self.workspace.glob("*.py")) or list(
...
class SecretScanner:
    ...
    def scan(self) -> list[SecretMatch]:
        ...
        try:
            # OPTIMIZATION: Replacing os.walk() with custom iter_files() generator.
            # Avoids allocating massive intermediate lists for dirs and files on every step,
            # and uses faster os.scandir for direct lazily yielded iterations.
            for file_path in iter_files(
                self.workspace, exclude_dirs=set(self.EXCLUDE_DIRS), include_hidden=True
            ):

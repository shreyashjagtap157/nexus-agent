...
class TestRunner:
    ...
    def detect_framework(self) -> str | None:
            ...
            for f in files:
                if (self.workspace / f).exists():
                    if fw == "pytest-fallback":
                        # Optimize: Use any() instead of list() for efficient existence checking
        ...
        # Optimize: Use any() instead of list() for efficient existence checking
        if any(self.workspace.glob("*.py")) or (
            (self.workspace / "src").exists() and any((self.workspace / "src").glob("**/*.py"))
        ):
            return "unittest"

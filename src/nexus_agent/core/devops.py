...
class TestRunner:
    ...
    def detect_framework(self) -> str | None:
            ...
            for f in files:
                if (self.workspace / f).exists():
                    if fw == "pytest-fallback":
                        # Bolt optimization: Use any() to prevent greedy generator evaluation
        ...
        # Bolt optimization: Use any() instead of list() for existence checks
...
class LinterRunner:
    ...
    def run_all(self) -> tuple[bool, str]:
        ...
        linter_cmds = []
        # Bolt optimization: Use any() to prevent greedy evaluation of matched paths

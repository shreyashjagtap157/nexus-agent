...
parse stack traces, scan for secrets, run dependency audits, and self-heal test failures.
"""
...
class TestRunner:
    ...
    def detect_framework(self) -> str | None:
            ...
            for f in files:
                if (self.workspace / f).exists():
                    if fw == "pytest-fallback":
                        # ⚡ Bolt: Fast glob lazy eval to check test files without memory bloat
                        if (self.workspace / "pyproject.toml").exists() or next(
                            (self.workspace / "tests").glob("**/*.py"), None
                        ) is not None:
                            return "pytest"
                    else:
        ...
        if next(self.workspace.glob("*.py"), None) is not None or (
            next((self.workspace / "src").glob("**/*.py"), None) is not None

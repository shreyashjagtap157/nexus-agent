...
class PluginManager:
    ...
    def register_tool(self, plugin_name: str, tool_instance: Any) -> None:
        ...
        if plugin_name in self.plugins:
            ...
            logger.info(
                f"Registered plugin tool: {getattr(tool_instance, 'name', type(tool_instance).__name__)} from {plugin_name}"  # noqa: E501
            )  # noqa: E501
    ...
    def _load_file_plugin(self, file_path: Path) -> None:
        ...
        try:
            ...
            if not registered:
                info.error = (
                    "No entry point ('register_plugin' function or 'NexusPlugin' subclass) found."  # noqa: E501
                )
                self.plugins[name] = info

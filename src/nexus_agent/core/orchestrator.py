...
class Orchestrator:
    ...
    def _parse_subtasks(self, plan: str) -> list[BoomerangSubTask]:
        ...
        for line in lines:
            ...
            if numbered_match:
                ...
                current_context = []
            elif line.strip().lower().startswith("context:") or line.strip().lower().startswith(
                "tools:"
            ):  # noqa: E501
    ...
    def run_task(self, task: str, plan_only: bool = False) -> Iterator[AgentEvent]:
        ...
        yield AgentEvent(
            type="content",
            data="[bold magenta]◆ Orchestrator spawned Planner Sub-Agent...[/bold magenta]\n",
        )  # noqa: E501
        ...
        if not plan_content:
            yield AgentEvent(
                type="error", data="Planner failed to generate an implementation plan."
            )  # noqa: E501
            return
        ...
        if self.approval_callback:
            ...
            yield AgentEvent(
                type="content", data="[yellow]⌛ Waiting for plan approval...[/yellow]\n"
            )  # noqa: E501
        ...
        if not approved:
            yield AgentEvent(
                type="content",
                data="[red]❌ Implementation plan was rejected by the user. Aborting task.[/red]\n",
            )  # noqa: E501
            yield AgentEvent(
                type="done", data={"plan": plan_content, "executed": False, "approved": False}
            )  # noqa: E501
            return
    ...
    def run_autonomous(self, goal: str) -> Iterator[AgentEvent]:
        ...
        yield AgentEvent(
            type="content",
            data=f"[bold magenta]◆ Initializing Autonomous Goal Graph (Session: {session_id})...[/bold magenta]\n",  # noqa: E501
        )  # noqa: E501

        # 1. Initialize and Decompose goal
        task_graph = TaskGraph(
            session_id=session_id, workspace=self.workspace, provider=self.provider
        )  # noqa: E501
        ...
        while True:
            ...
            if not ready_tasks:
                ...
                if progress["pending"] > 0 or progress["failed"] > 0 or progress["blocked"] > 0:
                    yield AgentEvent(
                        type="content",
                        data="[red]⚠️ Autonomous execution loop blocked or some sub-tasks failed.[/red]\n",  # noqa: E501
                    )  # noqa: E501
                    break
                else:
                    # All tasks complete
            ...
            for task in ready_tasks:
                ...
                while subtask_attempts < max_subtask_retries:
                    ...
                    if subtask_attempts > 1:
                        yield AgentEvent(
                            type="content",
                            data=f"[bold purple]🧠 NLA Telemetry Self-Healing Attempt {subtask_attempts - 1}/{max_subtask_retries}...[/bold purple]\n",  # noqa: E501
                        )  # noqa: E501
                    ...
                    if execution_err:
                        ...
                        yield AgentEvent(
                            type="content",
                            data="[cyan]🔍 Running local DevOps static validation & test verification...[/cyan]\n",  # noqa: E501
                        )  # noqa: E501
                        ...
                        if not report.tests_passed:
                            ...
                            yield AgentEvent(
                                type="content",
                                data="[cyan]⚖️ Convening parallel multi-agent expert review panel...[/cyan]\n",  # noqa: E501
                            )  # noqa: E501
                            ...
                            if (self.workspace / ".git").exists():
                                try:
                                    ...
                                    logger.warning(
                                        f"Failed to get git diff for debate context: {e}"
                                    )  # noqa: E501

                            verdict = debate_engine.run_debate(
                                code_changes=changes_text, context=task.description
                            )  # noqa: E501
                                    ...
                                    if matches:
                                        for match in matches:
                                            ...
                                            try:
                                                ...
                                                if not str(resolved).startswith(
                                                    str(self.workspace.resolve()) + os.sep
                                                ):  # noqa: E501
                                                    logger.warning(
                                                        f"Path traversal blocked in debate reworked code: {file_rel_path}"  # noqa: E501
                                                    )  # noqa: E501
                                                    continue
                                            ...
                                            if target_file.exists():
                                                ...
                                                yield AgentEvent(
                                                    type="content",
                                                    data=f"[green]✍️ Applied debate consensus refactoring to: {file_rel_path}[/green]\n",  # noqa: E501
                                                )  # noqa: E501
                                    else:
                                        ...
                                        if (self.workspace / ".git").exists():
                                            ...
                                            if status_res.returncode == 0:
                                                modified_files = [
                                                    f.strip()
                                                    for f in status_res.stdout.splitlines()
                                                    if f.strip()
                                                ]  # noqa: E501
                                        ...
                                        code_to_write = (
                                            ...
                                            if code_block_match
                                            else verdict.reworked_code.strip()
                                        )  # noqa: E501
                                        ...
                                        if len(modified_files) == 1 and code_to_write:
                                            ...
                                            try:
                                                ...
                                                if not str(resolved).startswith(
                                                    ...
                                                    logger.warning(
                                                        f"Path traversal blocked in debate modified file: {mf}"  # noqa: E501
                                                    )  # noqa: E501
                                                else:
                                                    target_file.write_text(
                                                        code_to_write, encoding="utf-8"
                                                    )  # noqa: E501
                                                    yield AgentEvent(
                                                        type="content",
                                                        data=f"[green]✍️ Applied debate consensus refactoring to: {mf}[/green]\n",  # noqa: E501
                                                    )  # noqa: E501
                                            except (OSError, ValueError):
                                                pass
                                except (OSError, ValueError) as write_ex:
                                    logger.warning(
                                        f"Failed to write reworked debate code to disk: {write_ex}"
                                    )  # noqa: E501
                    ...
                    if not failure_reason:
                        ...
                        yield AgentEvent(
                            type="content",
                            data=f"[bold green]✅ Sub-Task '{task.title}' completed successfully![/bold green]\n",  # noqa: E501
                        )  # noqa: E501
                        ...
                        yield AgentEvent(
                            type="content",
                            data=f"[red]❌ Attempt {subtask_attempts} failed: {failure_reason}[/red]\n",  # noqa: E501
                        )  # noqa: E501

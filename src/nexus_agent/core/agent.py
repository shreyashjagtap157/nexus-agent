...
"""
    ...
    def _log_iteration_telemetry(self, response: LLMResponse) -> None:
        try:
            self.nla_telemetry.log_iteration(
                ...
                if response.has_tool_calls
                else self.DEFAULT_FINISH_CONFIDENCE,  # noqa: E501
                alternative_paths=[],
            )
    ...
    def _handle_reflection(
    ...
    ) -> tuple[bool, list[AgentEvent]]:  # noqa: E501
        ...
        if not critique.approved:
            events.append(
                self._emit_event(
                    "thinking",
                    f"Reflection failed (Score: {critique.score}). Injecting self-correction prompt...",  # noqa: E501
                )
            )  # noqa: E501
            with self._lock:

...
class DebateEngine:
    ...
    def __init__(self, provider: LLMProvider | None = None, approval_threshold: int = 70):
        ...
        self.personas = [SECURITY_AUDITOR, PERFORMANCE_ENGINEER, CORRECTNESS_QA, STYLE_LINTER]

    def _review_single(
        self, persona: ReviewerPersona, code_changes: str, context: str
    ) -> CritiqueFeedback:  # noqa: E501
        ...
        try:
            ...
            response = self.provider.chat_completion(
                messages=messages, temperature=0.1, max_tokens=1500
            )
            ...
            logger.warning(
                f"Reviewer '{persona.name}' failed: {e}. Generating default review.", exc_info=True
            )  # noqa: E501
            return CritiqueFeedback(
                reviewer_name=persona.name,
                score=50,
    ...
    def _heuristic_review(self, code_changes: str) -> list[CritiqueFeedback]:
        ...
        feedbacks.append(
            CritiqueFeedback(
                ...
                sec_issues,
                sec_sug,
                "Rule-based security scan completed.",
            )
        )  # noqa: E501
        ...
        feedbacks.append(
            CritiqueFeedback(
                ...
                perf_issues,
                perf_sug,
                "Rule-based performance scan completed.",
            )
        )  # noqa: E501
        ...
        if "except:" in lower_code or "except Exception:" in lower_code:
            if "raise" not in lower_code and "pass" in lower_code:
                corr_issues.append(
                    "Silent exception catching detected (except without raise or log)."
                )  # noqa: E501
        ...
        feedbacks.append(
            CritiqueFeedback(
                ...
                corr_issues,
                corr_sug,
                "Rule-based correctness scan completed.",
            )
        )  # noqa: E501
        ...
        feedbacks.append(
            CritiqueFeedback(
                ...
                style_issues,
                style_sug,
                "Rule-based style linter completed.",
            )
        )  # noqa: E501
    ...
    def judge(self, reviews: list[CritiqueFeedback], rounds: int = 1) -> DebateVerdict:
        ...
        final_approved = consensus_score >= self.approval_threshold and all(
            r.approved for r in reviews
        )  # noqa: E501
    ...
    def run_debate(
        self, code_changes: str, context: str = "", max_rounds: int = 2
    ) -> DebateVerdict:  # noqa: E501
        """Run debate loop.
        ...
        for r_num in range(1, max_rounds + 1):
            ...
            if r_num < max_rounds and self.provider:
                ...
                logger.info(
                    f"Consensus failed in round {r_num} with score {verdict.consensus_score}. Reworking code changes..."  # noqa: E501
                )  # noqa: E501
                try:
                    system_prompt = (
...
class Council:
    ...
    def _rebuttal_round(
        ...
        for v in round1_votes:
            summary_lines.append(
                f"- {v.member_name}: {v.vote} (confidence {v.confidence:.1f}) — {v.reasoning[:200]}"
            )  # noqa: E501
        round1_summary = "\n".join(summary_lines)
            ...
            try:
                ...
                round2_votes.append(
                    CouncilVote(
                        ...
                        vote=vote_str,
                        reasoning=str(
                            data.get("reasoning", existing.reasoning if existing else "")
                        ),
                        confidence=min(
                            1.0,
                            max(
                                0.0,
                                float(
                                    data.get("confidence", existing.confidence if existing else 0.5)
                                ),
                            ),
                        ),  # noqa: E501
                    )
                ...
                round2_votes.append(
                    ...
                    or CouncilVote(
                        member_name=member.name,
                        vote="abstain",
                        reasoning=f"Rebuttal failed: {e}",
                        confidence=0.0,  # noqa: E501
                    )
                )
    ...
    def _tally(self, topic: str, votes: list[CouncilVote], duration: float) -> CouncilDecision:
        ...
        summary_lines = [
            ...
            f"Approve: {approve_pct:.0%}  |  Reject: {reject_pct:.0%}  |  Abstain: {weighted['abstain'] / total_weight:.0%}"  # noqa: E501
            if total_weight > 0
            else "",  # noqa: E501
            "",
        ]
    ...
    def _heuristic_votes(self, topic: str) -> list[CouncilVote]:
        ...
        for member in self.members:
            if member.role == "product_strategy" and ("migrate" in t or "refactor" in t):
                votes.append(
                    CouncilVote(member.name, "approve", "Strategic value in modernization.", 0.6)
                )  # noqa: E501
            elif member.role == "security_audit" and ("migrate" in t or "upgrade" in t):
                votes.append(
                    CouncilVote(
                        member.name, "approve", "Newer versions have fewer vulnerabilities.", 0.7
                    )
                )  # noqa: E501
            elif member.role == "execution_feasibility" and ("large" in t or "complex" in t):
                votes.append(
                    CouncilVote(member.name, "abstain", "Need more implementation detail.", 0.4)
                )  # noqa: E501
            else:
                votes.append(
                    CouncilVote(member.name, "approve", "Heuristic: default approve.", 0.5)
                )  # noqa: E501

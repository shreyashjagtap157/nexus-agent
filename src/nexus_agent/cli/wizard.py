...
class SetupWizard:
    ...
    def _detect_hardware(self) -> None:
        ...
        table.add_row(
            "Recommended Model Size", self.hardware.get("recommended_model_size", "unknown")
        )  # noqa: E501
        ...
        if install_now:
            if not RuntimeManager.is_runtime_installed(default_runtime):
                ...
                if success:
                    ...
                    self.console.print(
                        f"  [yellow]Installation skipped or failed. You can install later via /runtime install {default_runtime}[/yellow]"  # noqa: E501
                    )  # noqa: E501
            else:
                self.console.print(
                    f"  [green]✓ {default_runtime} runtime already installed[/green]"
                )  # noqa: E501
                self.config_updates.setdefault("local_model", {})
                self.config_updates["local_model"]["runtime"] = default_runtime
    ...
    def _recommend_model(self) -> None:
        ...
        self.console.print(
            f"Based on your hardware ([green]{hw.get('recommended_model_size', '?')}[/green]):"
        )  # noqa: E501
        self.console.print()

        if usable >= 64 * 1024**3:
            self.console.print(
                "  [bold]Large models (70B+):[/bold] llama-3.3-70b, qwen2.5-72b, deepseek-v2-67b"
            )  # noqa: E501
            recommended = "llama-3.3-70b (Q4_K_M)"
        elif usable >= 32 * 1024**3:
            self.console.print(
                "  [bold]Medium models (30B-70B):[/bold] llama-3.1-70b (Q4), mixtral-8x22b (Q4)"
            )  # noqa: E501
            recommended = "llama-3.1-70b (Q4_K_M)"
        elif usable >= 16 * 1024**3:
            self.console.print(
                "  [bold]Standard models (13B-30B):[/bold] llama-3.1-8b, qwen2.5-14b, phi-4-14b"
            )  # noqa: E501
            recommended = "llama-3.1-8b (Q4_K_M)"
        elif usable >= 8 * 1024**3:
            self.console.print(
                "  [bold]Small models (7B-13B):[/bold] qwen2.5-7b, gemma-2-9b, llama-3.2-3b"
            )  # noqa: E501
            recommended = "qwen2.5-7b (Q4_K_M)"
        else:
            self.console.print(
                "  [bold]Tiny models (1B-3B):[/bold] phi-3-mini, gemma-2-2b, qwen2.5-1.5b"
            )  # noqa: E501
        ...
        self.console.print(
            "  [dim]You can download GGUF models from: [cyan]https://huggingface.co/models?search=gguf[/cyan][/dim]"  # noqa: E501
        )  # noqa: E501

        download = self._confirm(
            "  Would you like to open the HF model search page?", default=False
        )  # noqa: E501
        if download:
            import webbrowser
    ...
    def _show_summary(self) -> None:
        ...
        items = [
            ...
            (
                "Guardrail Level",
                self.config_updates.get("local_model", {}).get("guardrails", "balanced"),
            ),  # noqa: E501
            (
                "GPU Offloading",
                "Enabled"
                if self.config_updates.get("local_model", {}).get("gpu_layers", 0) != 0
    ...
    def _save(self, provider_key: str | None) -> None:
        ...
        try:
            ...
            self.console.print()
            self.console.print(
                "[bold green]Setup complete![/bold green] Run [cyan]nexus chat[/cyan] to start."
            )  # noqa: E501
        except OSError as e:
            ...
            self.console.print()
            self.console.print(
                "[yellow]Setup incomplete. Run [cyan]nexus wizard[/cyan] to try again.[/yellow]"
            )  # noqa: E501

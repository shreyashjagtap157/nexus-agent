    ...
    def _find_files(self, prefix: str) -> list[str]:
        ...
        if not matches:
            try:
                ...
                def _scandir_prefix(path_str: str, pfx: str):
                    try:
                        for entry in os.scandir(path_str):
                            if entry.is_dir(follow_symlinks=False):
                                ...
                                if entry.name.startswith(".") or entry.name in skip:
                                    continue
                                yield from _scandir_prefix(entry.path, pfx)
                            elif pfx in entry.name:
                                yield entry.path
                    except OSError:
                        pass

                # ⚡ Bolt: Fast os.scandir traversal avoids pathlib.rglob overhead and uses lazy islice  # noqa: E501
                for p_str in itertools.islice(_scandir_prefix(str(self.workspace), prefix), 20):
                    rel = Path(p_str).relative_to(self.workspace)

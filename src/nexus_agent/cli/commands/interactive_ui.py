    ...
    def _find_files(self, prefix: str) -> list[str]:
        ...
        if not matches:
            try:
                ...
                def _fast_search(base_path: str):
                    try:
                            ...
                            for entry in it:
                                if entry.is_dir(follow_symlinks=False):
                                    if entry.name.startswith(".") or entry.name in {
                                        ...
                                        continue
                                    yield from _fast_search(entry.path)
                                elif entry.is_file() and prefix in entry.name:
                                    yield entry.path
                    except OSError:
                        pass

                for path_str in _fast_search(str(self.workspace)):
                    from pathlib import Path

                    rel = Path(path_str).relative_to(self.workspace)

    ...
    def _find_files(self, prefix: str) -> list[str]:
        ...
        if not matches:
            try:
                ...
                from nexus_agent.utils.fs import fast_scandir

                # ⚡ Bolt: Using fast_scandir instead of Path.rglob() to avoid Path object allocation overhead  # noqa: E501
                for p_str in fast_scandir(str(self.workspace)):
                    rel_str = str(Path(p_str).relative_to(self.workspace).as_posix())
                    if prefix in rel_str or prefix_lower in rel_str.lower():

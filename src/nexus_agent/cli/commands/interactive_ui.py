...
class InteractiveUIMixin:
    ...
    def _find_files(self, prefix: str) -> list[str]:
        ...
        if not matches:
            try:
                # ⚡ Bolt: Replace slow pathlib.rglob() with os.scandir() using explicit stack for file iteration to avoid memory overhead  # noqa: E501
                        ...
                        with os.scandir(current_dir) as it:
                            for entry in it:
                                ...
                                if entry.is_dir(follow_symlinks=False):
                                    ...
                                    # Use relative path string for matching
                                    try:
                                        rel_path = Path(entry.path).relative_to(self.workspace)

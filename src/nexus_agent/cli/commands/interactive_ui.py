...
class InteractiveUIMixin:
    ...
    def _interactive_menu(
        ...
        *items* is a list of ``(label, value)`` pairs.  Pairs with
        ...
        """
        selectable = [i for i, (_, v) in enumerate(items) if v is not None]
        if not selectable:
            return None
        idx = selectable[0]
    ...
    def _find_files(self, prefix: str) -> list[str]:
        ...
        if not matches:
            try:
                # ⚡ Bolt: Use islice for lazy rglob evaluation and early exit
                for p in itertools.islice(
                    self.workspace.rglob(f"*{prefix}*"), 1000
                ):  # Cap search at 1000 files to prevent unbounded iteration delay
                    if p.is_file():

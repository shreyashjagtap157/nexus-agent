...
class InteractiveUIMixin:
    ...
    def _find_files(self, prefix: str) -> list[str]:
        ...
        if not matches:
            ...
            def _fast_prefix_search(path: str, prefix_str: str):
                try:
                        ...
                        for entry in it:
                            if entry.is_dir(follow_symlinks=False):
                                if not entry.name.startswith(".") and entry.name not in {
                                    ...
                                    yield from _fast_prefix_search(entry.path, prefix_str)
                            elif entry.is_file(follow_symlinks=False) and prefix_str in entry.name:
                                yield entry.path
                except OSError:
                    pass

            try:
                # ⚡ Bolt: Use os.scandir generator instead of rglob for significantly faster path matching  # noqa: E501
                for file_path in _fast_prefix_search(str(self.workspace), prefix):
                    try:

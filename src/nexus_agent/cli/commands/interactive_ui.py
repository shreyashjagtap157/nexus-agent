    ...
    def _find_files(self, prefix: str) -> list[str]:
        ...
        if not matches:
            try:
                ...
                def fast_glob_prefix(root, limit=20):
                    import os
                        ...
                        with os.scandir(root) as it:
                            for entry in it:
                                ...
                                if entry.is_dir(
                                    ...
                                    res.extend(fast_glob_prefix(entry.path, limit - len(res)))
                                elif entry.is_file(follow_symlinks=False) and prefix in entry.name:
                                    rel = Path(entry.path).relative_to(self.workspace)
                                    res.append(str(rel.as_posix()))
                    except OSError:
                        pass
                    return res

                matches.extend(fast_glob_prefix(str(self.workspace), 20 - len(matches)))

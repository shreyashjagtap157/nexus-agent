...
class AgentCommandsMixin:
    ...
    def _cmd_memory_help(self):
        ...
        if self._memory:
            ...
            self.console.print(
                "  [dim]  /memory vector migrate            Re-embed all FTS5 memories into vector \
                store[/dim]"
            )
            self.console.print(
    ...
    def _get_vector_store(self) -> typing.Any | None:
    ...
    def _get_memory_manager(self) -> typing.Any | None:
    ...
    def _cmd_memory_vector(self, args: str):
        ...
        if not stripped:
            self.r.system_message(
                ...
                "filter <category> | migrate | download | delete <entry_id> | "
                "clear | rebuild"
            )
            return
        ...
        if subcmd == "stats":
            ...
            self.r.system_message(
                "Usage: /memory vector stats | query <text> | list [N] | "
                "filter <category> | categories | migrate | download | delete <entry_id> | "
                "clear | rebuild"
            )
    ...
    def _cmd_memory_vector_stats(self, args: str = ""):
        ...
        try:
            ...
            if mode == "onnx" and engine:
                ...
                self.console.print(
                    "  [dim]ℹ Using ngram fallback - run "
                    "/memory vector download for ONNX model[/dim]"
                )
    ...
    def _cmd_memory_vector_rebuild(self):
        ...
        try:
            ...
            if before == 0:
                self.r.system_message(
                    "Vector store is empty — nothing to rebuild. \
                    Run /memory vector migrate to populate from FTS5."
                )
                return
    ...
    def _cmd_memory_vector_clear(self):
        ...
        try:
            ...
            label = "project" if getattr(self, "_vector_use_project", False) else "global"
            self.r.system_message(
                f"Cleared {deleted} vector embeddings ({label}). FTS5 memories untouched. \
                Run /memory vector migrate to re-populate."
            )
        except Exception as exc:
            ...
            while offset < total:
                ...
                for entry in entries:
                    ...
                    if migrated % 10 == 0 or (migrated + skipped) % page_size == 0:
                        ...
                        bar = "█" * (pct // 4) + "░" * (25 - pct // 4)
                        self.console.print(
                            f"    [{bar}] {done}/{total} ({pct}%)  [green]+{migrated}[/green] new  \

...
class AgentCommandsMixin:
    ...
    def _cmd_memory_vector(self, args: str):
        ...
        if not stripped:
            self.r.system_message(
                "Usage: /memory vector [--project] stats | query <text> | list [N] | filter "
                "<category> | migrate | download | delete <entry_id> | clear | rebuild"
            )
            return
        ...
        if subcmd == "stats":
            ...
            self.r.system_message(
                "Usage: /memory vector [--project] stats | query <text> | list [N] | filter "
                "<category> | categories | migrate | download | delete <entry_id> | clear | "
                "rebuild"
            )

...
class AgentCommandsMixin:
    ...
    def _render_effort_selector(self, levels: tuple, labels: tuple, idx: int):
        ...
        lines = [
            ...
            (f"{' ' * pad}{chr(0x2500) * left_w}" + chr(0x252C) + f"{chr(0x2500) * right_w}"),
        ]
    ...
    def _cmd_memory_vector_stats(self, args: str = ""):
        ...
        try:
            ...
            if mode == "onnx" and engine:
                ...
                self.console.print(
                    "  [dim]ℹ Using ngram fallback — run /memory vector download "
                    "for ONNX model[/dim]"
                )
    ...
    def _cmd_memory_vector_list(self, args: str):
        ...
        try:
            ...
            self.console.print(
                    ...
                    f"  [bold]Vector store ({label}):[/bold] [green]{total}[/green] "
                    f"total entries, showing [cyan]{min(show, len(entries))}[/cyan]"
    ...
    def _cmd_memory_vector_categories(self):
        ...
        try:
            ...
            self.console.print()
            self.console.print(
                f"  [bold]Categories ({label}):[/bold] [green]{len(cats)}[/green] "
                f"unique, [cyan]{total}[/cyan] total entries"
            )
            self.console.print()
        ...
        try:
            ...
            for i, r in enumerate(results, 1):
                ...
                self.console.print(
                    ...
                    f"[{score_color}]{pct:>2}%[/{score_color}]  "
                    f"[{cat}] {content}"
                )

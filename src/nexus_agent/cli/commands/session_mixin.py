                        ...
                        for entry in it:
                            if entry.is_dir(follow_symlinks=False):
                                if not entry.name.startswith(".") and entry.name not in {
                                    ...
                                    yield from _fast_py_files(entry.path)
                            elif entry.name.endswith(".py"):
                                yield entry.path
                except OSError:
                    pass

            # ⚡ Bolt: Replace rglob with fast os.scandir traversal to avoid Path object overhead
            files = list(itertools.islice(_fast_py_files(str(self.workspace)), 20))

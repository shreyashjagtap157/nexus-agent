...
class SessionCommandsMixin:
    ...
    def _cmd_checkpoint(self, args: str):
        if self._session_mgr:
            # ⚡ Bolt: Lazily evaluate file generator with itertools.islice to prevent loading entire tree into memory  # noqa: E501
            files = list(itertools.islice((str(f) for f in self.workspace.rglob("*.py")), 20))

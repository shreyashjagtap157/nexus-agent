...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    # Protect against CSWSH by verifying Origin header matches Host
    origin = websocket.headers.get("origin")
    host_header = websocket.headers.get("host")
    if origin and host_header:
        parsed_origin = urllib.parse.urlparse(origin).hostname

        # Handle IPv6 brackets in host header
        ...
        if parsed_origin != host_hostname:
            # Close connection if cross-site WebSocket hijack attempt detected

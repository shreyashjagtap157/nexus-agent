...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if origin is not None:
        ...
        if host_header.startswith("["):
            host_name = host_header[1 : host_header.find("]")]
        else:
            host_name = host_header.split(":")[0]
        parsed_origin = urllib.parse.urlparse(origin)
        if parsed_origin.hostname != host_name and parsed_origin.hostname not in (
            "127.0.0.1",
            "localhost",
        ):
            raise WebSocketException(
                code=status.WS_1008_POLICY_VIOLATION, reason="CSWSH check failed"
            )

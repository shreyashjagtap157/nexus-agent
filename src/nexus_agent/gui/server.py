...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if origin is not None:
        if origin == "null":
            logger.warning("Rejected WebSocket connection: 'null' Origin.")
        ...
        if host_header.startswith("["):
            parsed_host = host_header[1 : host_header.find("]")]
        else:
            parsed_host = host_header.split(":")[0]

        if parsed_origin != parsed_host:
            logger.warning(f"CSWSH blocked: Origin {parsed_origin} != Host {parsed_host}")
            raise WebSocketException(code=1008, reason="CSWSH attack detected")

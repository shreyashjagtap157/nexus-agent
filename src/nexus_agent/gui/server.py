...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    # CSWSH protection
    # Allow programmatic clients that don't send an Origin header
    if origin is not None:
        if origin == "null" or not host_header:
            logger.warning(
                "Rejected WebSocket connection due to invalid origin or missing host header. "
                f"Origin: {origin}, Host: {host_header}"
            )
            await websocket.close(code=1008)
            return

        origin_hostname = urllib.parse.urlparse(origin).hostname
        host_hostname = urllib.parse.urlparse(f"http://{host_header}").hostname

        if origin_hostname != host_hostname:
            logger.warning(
                "Rejected WebSocket connection due to CSWSH protection. "
                f"Origin: {origin_hostname} vs Host: {host_hostname}"
            )
            await websocket.close(code=1008)
            return
                ...
                await websocket.send_json(
                    {
                        ...
                        "Please load a model or configure a provider first.",
                    }
                )

    ...
    if origin is not None:
        ...
        def extract_hostname(host_str: str) -> str:
            if host_str.startswith("["):
                return host_str[1 : host_str.find("]")]
            return host_str.split(":")[0]

        if extract_hostname(parsed_origin.netloc) != extract_hostname(host_header):
            await websocket.close(code=1008)
            return
        ...
        while True:
            ...
            if not engine or not is_loaded:
                await websocket.send_json(
                    {
                        "type": "error",
                        "content": "No model loaded. Load a model or configure a provider first.",
                    }
                )

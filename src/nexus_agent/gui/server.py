...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    # Reject completely unauthenticated origin (e.g. local html file)
    if origin == "null":
    ...
    if origin is not None and host_header is not None:
        ...
        origin_no_port = parsed_origin.split(":")[0]

        # We allow same-origin. If origin doesn't match host, reject.

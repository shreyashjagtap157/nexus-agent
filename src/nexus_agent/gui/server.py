...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    # Browsers do not enforce Same-Origin Policy on WebSockets.
    # We must explicitly validate the Origin header against the Host header.
    if origin is not None:
        # Reject "null" origins to prevent bypasses via data URIs or local HTML files.
        if origin == "null":
        ...
        while True:
            ...
            if not engine or not is_loaded:
                await websocket.send_json(
                    {
                        "type": "error",
                        "content": "No model loaded. Please load a model.",
                    }
                )

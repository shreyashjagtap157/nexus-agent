...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if origin is not None:
        if origin == "null":
            logger.warning("CSWSH attempt blocked: origin is null")
            await websocket.close(code=1008, reason="CSWSH protection: Invalid origin")
            return

        parsed_origin = urllib.parse.urlparse(origin)
        origin_hostname = parsed_origin.hostname
        host_hostname = host.split(":")[0] if host else ""

        if origin_hostname != host_hostname:
            logger.warning(f"CSWSH blocked: origin {origin_hostname} != host {host_hostname}")
            await websocket.close(code=1008, reason="CSWSH protection: Origin mismatch")
            return

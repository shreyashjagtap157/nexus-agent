    ...
    if origin is not None:
        if origin == "null":
            await websocket.close(
                code=1008, reason="Cross-Site WebSocket Hijacking (CSWSH) protection triggered."
            )
            return
        ...
        if host_header.startswith("["):
            end_idx = host_header.find("]")
            expected_hostname = host_header[1:end_idx] if end_idx != -1 else host_header
        else:
            expected_hostname = host_header.split(":")[0]

        if origin_hostname != expected_hostname:
            await websocket.close(
                code=1008, reason="Cross-Site WebSocket Hijacking (CSWSH) protection triggered."
            )
            return
                ...
                await websocket.send_json(
                    {
                        "type": "error",
                        "content": (
                            "No model loaded. Please load a model or configure a "
                            "provider first."
                        ),
                    }
                )

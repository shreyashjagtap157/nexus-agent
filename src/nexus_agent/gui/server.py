...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if origin is not None:
        if origin == "null":
            raise WebSocketException(code=1008, reason="CSWSH: null origin")
        ...
        if host:
            ...
            if origin_hostname != host_hostname:
                raise WebSocketException(code=1008, reason="CSWSH: origin mismatch")
    ...
    try:
        while True:
            ...
            if not engine or not is_loaded:
                await websocket.send_json(
                    {
                        "type": "error",
                        "content": "No model loaded. Please configure a provider.",  # noqa: E501
                    }
                )
        ...
        logger.warning(f"WebSocket policy violation: {we.reason}")

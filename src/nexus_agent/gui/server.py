    ...
    if origin is not None:
        if origin == "null":
            raise WebSocketException(
                code=status.WS_1008_POLICY_VIOLATION, reason="CSWSH protection: Invalid Origin"
            )
        ...
        if origin_hostname != host_hostname:
            raise WebSocketException(
                code=status.WS_1008_POLICY_VIOLATION, reason="CSWSH protection: Origin mismatch"
            )

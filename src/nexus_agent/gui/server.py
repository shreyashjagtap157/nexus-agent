...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    # CSWSH Protection: Validate Origin header against allowed local hosts
    ...
    if origin:
        try:
            ...
            # Allow if host header matches origin netloc OR if origin hostname is in allowed_hosts
            if not host_header or (
                parsed_origin.netloc != host_header and parsed_origin.hostname not in allowed_hosts
            ):
            ...
            logger.warning(f"Failed to parse WebSocket origin: {e}")
            await websocket.close(code=1008, reason="Invalid origin format")
            return

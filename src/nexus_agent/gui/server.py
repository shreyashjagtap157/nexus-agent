    ...
    if origin is not None:
        if origin == "null":
            raise WebSocketException(
                code=1008, reason="Cross-Site WebSocket Hijacking blocked (null origin)"
            )
        ...
        if parsed_origin != host_hostname:
            raise WebSocketException(code=1008, reason="Cross-Site WebSocket Hijacking blocked")
    ...
    try:
        while True:
            ...
            if len(data_str) > 65536:
                raise WebSocketException(code=1009, reason="Message too big")

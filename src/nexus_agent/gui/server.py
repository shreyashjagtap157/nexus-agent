...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if origin is not None:
        if origin == "null":
            raise WebSocketException(code=status.WS_1008_POLICY_VIOLATION, reason="Invalid Origin")
        parsed_origin = urllib.parse.urlparse(origin).hostname

        if host_header and host_header.startswith("["):
        ...
        while True:
            ...
            if len(data_str) > 65536:
                raise WebSocketException(
                    code=status.WS_1009_MESSAGE_TOO_BIG, reason="Message too big"
                )

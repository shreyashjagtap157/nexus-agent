...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if origin is not None:
        ...
        if host_header.startswith("["):
            host_header_name = host_header[1 : host_header.find("]")]
        else:
            host_header_name = host_header.split(":")[0]

        origin_name = parsed_origin.hostname
        if origin_name != host_header_name:
            logger.warning(
                f"CSWSH protection: rejected connection from origin {origin} "
                f"(expected host {host_header_name})"
            )
            await websocket.close(code=1008, reason="CSWSH check failed")
            return
        ...
        while True:
            ...
            if not engine or not is_loaded:
                await websocket.send_json(
                    {
                        "type": "error",
                        "content": "No model loaded. Please load a model or "

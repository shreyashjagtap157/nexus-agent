...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if origin is not None:
        if origin == "null":
            raise WebSocketException(code=1008, reason="Forbidden: Origin null not allowed")

        parsed_origin = urllib.parse.urlparse(origin)
        origin_hostname = parsed_origin.hostname

        if not host_header:
            raise WebSocketException(code=1008, reason="Forbidden: Missing Host header")
        ...
        if origin_hostname != host_hostname:
            raise WebSocketException(code=1008, reason="Forbidden: Cross-Site WebSocket Hijacking")
        ...
        while True:
            ...
            if not engine or not is_loaded:
                await websocket.send_json(
                    {
                        "type": "error",
                        "content": "No model loaded. Please load a model or configure a provider "
                        "first.",
                    }
                )

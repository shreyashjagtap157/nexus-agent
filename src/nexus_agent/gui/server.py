...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    # CSWSH Protection: Validate Origin header before accepting connection
    ...
    if origin:
        ...
        try:
            ...
            if parsed.hostname not in ("127.0.0.1", "localhost"):
                logger.warning(f"Blocked WebSocket connection from untrusted origin: {origin}")
                await websocket.close(code=1008, reason="Forbidden Origin")
                return
        except ValueError:
            logger.warning("Blocked WebSocket connection with malformed origin")
            await websocket.close(code=1008, reason="Forbidden Origin")
            return
    ...
    try:
        while True:
            ...
            if not engine or not is_loaded:
                msg = "No model loaded. Please load a model or configure a provider first."

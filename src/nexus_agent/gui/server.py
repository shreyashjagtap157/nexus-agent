...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if origin is not None:
        if origin == "null":
            ...
            return
        parsed_origin_host = urllib.parse.urlparse(origin).hostname
        if host:
            host_host = host[1 : host.find("]")] if host.startswith("[") else host.split(":")[0]
            if parsed_origin_host != host_host:
                await websocket.close(code=1008, reason="Origin not allowed")
                return

...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if origin is not None:
        ...
        # Extract hostname from Origin netloc
        if origin_netloc.startswith("["):
            origin_hostname = origin_netloc[1 : origin_netloc.find("]")]
        else:
            origin_hostname = origin_netloc.split(":")[0]
        ...
        while True:
            ...
            if not engine or not is_loaded:
                await websocket.send_json(
                    {
                        "type": "error",
                        "content": "No model loaded. Please load a model or provider first.",
                    }
                )

...
class LocalModelConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.chat_format not in valid_chat_formats:
            raise ValueError(
                f"Invalid chat_format '{self.chat_format}'. Must be one of {valid_chat_formats}"
            )
...
class RuntimeManager:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        try:
            ...
            self._validated_config = LocalModelConfig(
                **{k: v for k, v in filtered.items() if k in known_keys}
            )
    ...
    def select_engine(self, model_path: str | None = None) -> LLMProvider:
        ...
        if not target_path:
            raise ValueError(
                "No model path specified. Set default_model in config or provide model_path."
            )
        ...
        if runtime == "auto":
            if (
                ...
                and (resolved_path / "genai_config.json").exists()
            ):
                runtime = "onnx"
            else:
                runtime = "llama-cpp"
        ...
        try:
            if runtime == "onnx":
                if not ONNX_AVAILABLE:
                    logger.warning(
                        "ONNX Runtime is requested but not available. Falling back to llama.cpp."
                    )
                    runtime = "llama-cpp"
                else:
            ...
            logger.error(
                f"Failed to load new local engine runtime: {e}. Rolling back to old engine."
            )
    ...
    def activate_runtime(backend: str) -> None:
        ...
        if os.path.exists(target_dir):
            ...
            if target_dir_abs not in sys.path:
                ...
                logger.info(
                    f"Activated isolated runtime backend '{backend}' in sys.path: {target_dir_abs}"
                )
    ...
    def get_recommended_runtimes() -> list[str]:
        ...
        has_nvidia = bool(
            ...
            or shutil.which("nvidia-smi.exe")
    ...
    def is_runtime_installed(backend: str) -> bool:
        ...
        if backend in ("cpu", "cuda", "vulkan", "metal", "rocm"):
            ...
            return (target_dir / "llama_cpp").exists() or next(
                target_dir.glob("llama_cpp*"), None
            ) is not None
        if backend == "onnx":
            if not target_dir.exists():
                return False
            # ⚡ Bolt: Fast lazy evaluation instead of list(glob) for quick matching
            return (target_dir / "onnxruntime_genai").exists() or next(
                target_dir.glob("onnxruntime_genai*"), None
            ) is not None
    ...
    @staticmethod
    def install_runtime(
        backend: str, force_reinstall: bool = False, progress_callback: Any = None
    ) -> bool:
        ...
        if not rt:
            raise ValueError(
                f"Unknown runtime backend: {backend}. Valid: {list(INSTALLABLE_RUNTIMES)}"
            )
        ...
        if runtime_type == "external_server":
            install_guides = {
                "ollama": "Visit https://ollama.com to install, then run: ollama pull <model>",
                "lm_studio": "Download from https://lmstudio.ai and enable local API server.",
                "koboldcpp": "Download from LostRuins/koboldcpp and run with --openai-compat.",
            }
        ...
        if not force_reinstall and RuntimeManager.is_runtime_installed(backend):
            logger.info(
                f"Runtime {rt['name']} is already installed. Use force_reinstall=True to reinstall."
            )
            if progress_callback:
